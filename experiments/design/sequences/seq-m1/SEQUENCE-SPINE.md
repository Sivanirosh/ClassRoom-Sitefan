...
**Current gate:** replay each distinct low-fi `MES-01` catalog exercise independently. On 2026-07-17 the owner clearly approved continuing EX-0004 into UI/art expansion; that approval applies only to EX-0004 in the current `SEQ-M1` / `MES-01` bridge context. EX-0005 still needs its own replay and natural-language go-ahead. Hard implementation boundaries should be expressed through validated Khazad-Doom Issue Slices, not an exact approval phrase.
