...
- The active workflow pilot is now the bounded measurement pair `MES-01 → MES-02`; its [`SEQUENCE-SPINE.md`](../sequences/seq-m1/SEQUENCE-SPINE.md) `r0-bounded-measurement-pair` and separate [`MES-01` learning contract](../sequences/seq-m1/mes-01/LEARNING-BRIEF.md) `r0-unit-iteration-contract` are owner-approved. Semantic-fit candidates and implementation remain pending.
