<script lang="ts">
  ...
  const smokeSuccessPlans: Record<string, string> = {
    demarrage: 'command:1,execute',
    essai: 'command:10,command:1000,execute',
    passage: 'command:1,execute',
    bug: 'clear,command:100,execute',
          ...
          <article class="test-card">
            ...
            {#if view === 'question'}
              <section class="programmer" aria-label="Séquenceur de commandes">
                <div class="programmer-head"><div><span>Programme</span><strong>{program.length} / {scene.maxSlots} emplacements</strong></div><button class="text-button" type="button" data-smoke-control="clear" onclick={clearProgram} disabled={program.length === 0}>Tout effacer</button></div>
                ...
                <div class="command-rack" aria-label="Commandes disponibles">
                  ...
                  <div>{#each scene.available as command}<button type="button" data-smoke-control={`command:${command}`} onclick={() => addCommand(command)} disabled={program.length>=scene.maxSlots}><code>{commandCode(command)}</code><strong>{commandLabel(command)}</strong></button>{/each}</div>
                </div>
                <div class="run-controls"><button class="secondary" type="button" data-smoke-control="step" onclick={step}>Pas à pas</button><button class="primary" type="button" data-smoke-control="execute" onclick={execute}>Exécuter le programme <span aria-hidden="true">▶</span></button></div>
                <div class="program-message" role="status" data-smoke-feedback={consoleMessageKind === 'error' ? 'error' : undefined} data-smoke-feedback-detail={consoleMessageKind === 'error' ? 'true' : undefined}><span aria-hidden="true">›_</span><p>{consoleMessage}</p></div>
